...
export default function Home() {
  ...
  const translate = async (text) => {
    try {
      const textRes = await axios.post(window.location.href + "/api/hello", {
        text,
      });
      setTranslated(textRes.data.text);
    } catch (err) {
      console.log(err);
    }
  };
  ...
  return (
    ...
    >
      <div className="w-full my-4">
        <h1 className="text-blue-300 text-2xl mb-6">Metni Yapıştırın</h1>
        <textarea
      ...
      <div className="w-full flex flex-col my-8">
        <h1 className="text-blue-300 text-2xl mb-6">Düzeltilmiş Metin</h1>
        <textarea
      ...
      {translated !== "" && (
        <div className="w-full flex flex-col my-4">
          <h1 className="text-blue-300 text-2xl mb-6">Çevrilmiş Metin</h1>
          <textarea
            className={`w-full bg-slate-50 h-64 text-blue-500 p-2`}
            value={translated}
            readOnly
